...
```{r ans1b1}
as.factor(classSurvey$`性別`)->classSurvey$`性別` #性別加``
```
...
```{r ans1b3}
hwGrades$`學號` %in% classSurvey$`學號` #學號加``
```
...
```{r ans1b4}
which(classSurvey$`學號`=="4104d245c7") #學號加``
```
...
```{r ans1b5}
hwGrades$Exercise3[which(hwGrades$學號=="4104d245c7")] #which() #[]找出row的值
```
...
```{r ans12}
sum(str_detect(classSurvey$`課外活動`,"團隊活動")) #str_detect()查課外活中有多少人勾選團隊活動再用sum()加總
```
...
```{r ans132}
classSurvey[isFemale,]->subsample #[]選出純女性
```
...
```{r ans16}
hwGrades[which(hwGrades$`學號` %in% classSurvey$`學號`),]
```
...
```{r ans17}
classSurvey$學號[isFemale] -> 女性學號 #選出女性
hwGradesSubsample$女性 <- hwGradesSubsample$學號 %in% 女性學號  #%in%屬於
hwGradesSubsample
```
...
hwGradesSubsample$有團活 <- hwGradesSubsample$學號 %in% 有團隊的學號 #%in%
hwGradesSubsample$平均成績[(hwGradesSubsample$女性==T & hwGradesSubsample$有團活==T)] %>%  #選出女性及有團活
  mean -> meanFeAct
meanFeAct
```
...
hwSubmit %>%
  mutate(
    ...
    Timestamp=with_tz(Timestamp,tzone="Asia/Taipei"), #mutate創新變數
    學號=as.character(學號)
  ) -> hwSubmit
hwSubmit
```
...
```{r ans22}
deadline3 <- ymd_hms("2018-11-06 00:00:00",tz="Asia/Taipei") #設台北時區的期限
hwSubmit3 <- hwSubmit[hwSubmit$作業代碼=="Exercise3",]
hwSubmit3 %>%
  mutate(
    截止時差=difftime(deadline3,Timestamp,units="hours") #算時差
  ) ->  hwSubmit3
hwSubmit3
```
...
```{r ans23}
hwSubmit3$截止時差[hwSubmit3$女性==T] %>% mean -> FemaleTime  #取出女平均
hwSubmit3$截止時差[hwSubmit3$女性==F] %>% mean -> MaleTime    #取出男平均
FemaleTime-MaleTime -> dftime1 #女減男
dftime1
```
